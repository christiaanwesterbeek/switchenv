...
read()
  .then((entries) => {
    const filteredEntries = entries
      .filter(entry => entry.fields && entry.fields.Title === keepassEntryTitle)
      .map(entry => entry.fields.Notes )

    if (filteredEntries.length !== 1) {
      throw new Error(`Exactly 1 entry expected, but ${filteredEntries.length} are found by title ${keepassEntryTitle}`);
    }
